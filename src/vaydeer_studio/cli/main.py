...
def _protocol(mock: bool) -> VaydeerProtocol:
    ...
    return VaydeerProtocol(HidApiCommandTransport(interface.path))
...
def cmd_diagnostics(args: argparse.Namespace) -> int:
    ...
    payload = {
        "interfaces": [
            interface.__dict__ | {"report_descriptor": interface.report_descriptor.hex()}
            for interface in discover_linux_hidraw()
        ],
        "keepalive": manager.status(),
    }
